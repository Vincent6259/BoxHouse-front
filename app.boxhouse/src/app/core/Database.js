...
export class Database {
    ...
    async getAll(args){
        ...
        //        response = await network.promiseGet( settings.url.backend+'/'+args.collection+'/'+args.pk_id )
        //        single = response
        //    }
        //    // [OFFLINE]
        //    else{ alert('Vous ĂŞtes actuellement hors-ligne. Les donnĂ©es ne seront peut ĂŞtre pas accessibles.') }
        //}
    ...
    async update(args){
        ...
        //for( let i=0; i<this[args.collection].length; i++ ){
        //    if( this[args.collection][i].pk_id == args.pk_id ) this[args.collection][i] = {...args.data}
        //}

        // // [ONLINE] Send data to API
        // if( window.navigator.onLine == true ){
        //     response = await network.promisePut( settings.url.backend+'/'+args.collection+'/'+args.pk_id , args.data )
        // }
        //
        // // [OFFLINE] Add data to sync queue
        // else{
        //     this.tmp.push(function(){ network.promisePut( settings.url.backend+'/'+args.collection+'/'+args.pk_id , args.data ) })
        // }
    ...
    async create(args){
        ...
        // Add data to local database
        //this[args.collection].push( args.data )
    ...
    async delete(args){
        ...
        //this[args.collection] = this[args.collection].filter(function(el){ return el.pk_id!=args.pk_id }) // Remove from local database

        // [ONLINE] Send data to API
        // if( window.navigator.onLine == true )
        //     response = await network.promiseDelete( settings.url.backend+'/'+args.collection+'/'+args.pk_id )
        //
        // // [OFFLINE] Add data to sync queue
        // else if( window.navigator.onLine == false )
        //     this.tmp.push(function(){ network.promiseDelete( settings.url.backend+'/'+args.collection+'/'+args.pk_id ) })
        // Params
